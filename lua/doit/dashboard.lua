...
function M.render()
  ...
  -- Left column: Events
  table.insert(left_col, "TODAY'S EVENTS")
  table.insert(left_col, '──────────────')
  if doit.calendar then
    ...
    local events = {}
    if ok then
      local success, result = pcall(icalbuddy.get_events, today, end_date, doit.calendar.config and doit.calendar.config.icalbuddy or {})
      if success then
        events = result or {}
      end
    end
  ...
  table.insert(right_col, 'COMMANDS')
  ...
  table.insert(right_col, '')
  table.insert(right_col, 'PLUGINS')
  table.insert(right_col, '───────')
  ...
  if #registry_modules > 0 then
    ...
    table.insert(content, center('───────', width))
  ...
  table.insert(content, '')
  table.insert(content, '')
  table.insert(content, center("Press 'q' or ESC to close", width))
